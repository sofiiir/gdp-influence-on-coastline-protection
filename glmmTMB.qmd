...
### Confidence Interval
Since the Beta Regression models do not allow us to create confidence intervals using the predict function we get to do it by hand :))))))))). 
...
beta3_warmwater_coral <- sum_coastal_ecosystem_mod$coefficients$cond[6]
...
# Simulating data
We simulate data to make sure we understand the model before using the model on out 
```{r}
beta0 <- 0.6
beta1 <- 1.9
mean_gdp_2016 <- 14299.45
...
sim_df <- tibble(proportion_protected_sim, gdp , total_area)
