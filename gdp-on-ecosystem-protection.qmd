...
The beta regression model that was run on the simulated data outputs values close to those values we originally assigned to the variables. This is a sign we understand our model enough to move onto fitting the model on our own data!
...
The confidence intervals on the predictions are showing the confidence intervals when all of the variables influences are accounted for in the computation. The confidence intervals for the mangroves, salt marshes, sea grasses, and warm-water corals are all overlapping. This shows that the differences between the effect of per capita GDP on the protection of these ecosystems do not greatly differ from each other.

## Individual Beta's Confidence Intervals
