...
RE-RUN THIS FOR ALL OF THE DIFFERENT ECOSYSTEM TYPES.
```{r}
...
boot <-  map_dfr(1:1000, \(i) {
  ...
  boot_mod <- betareg(adj_proportion_protected ~ pc_gdp_2016 + total_area + ecosystem, 
                      data = boot_sample)
  
  boot_stat <- summary(boot_mod) 
  
  out <- boot_stat$coefficients$mean[ ,1]
...
```{r}
coastal_ecosystem_model$coefficients$cond[]
```


```{r}
# with(boot$pc_gdp_2016, qnorm(0.025, mean = mean(boot$pc_gdp_2016), sd = sd(boot$pc_gdp_2016)))
...
# coastal_ecosystem_model$coefficients[]
...
# proportion_protected_sim <- rbeta(100000, mu = mu, phi = phi)
```

IF THE PREDICT FUNCTION WORKED THIS IS HOW CONFIDENCE INTERVALS WOULD BE CALCULATED. 
```{r}
#| eval: false
