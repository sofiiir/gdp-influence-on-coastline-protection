...
# Purpose
This repository investigates the hypothesis that nations with higher per capita GDP are more likely to protect their coastlines. This hypothesis is derived from the general knowledge that protecting ecosystems takes a large amount of resources. Policy must be put in place to create these protected areas. Countries with a higher GDP per capita would have a greater amount of resources to spend on creating these zones. 
...
ggplot(data = coastal_ecosystem, aes(x = pc_gdp_2016, 
                                     ...
                                     color = ecosystem)) +
  geom_point() +
  scale_color_manual(values = c("#D51977", 
                                "forestgreen", 
                                "#273E47", 
                                "#9BC1BC", 
                                "#FEA82F"))+
...
### Confidence Interval
We can use the predict function to calculate confidence intervals. Unfortunately, the Beta Regression models do not allow us to create confidence intervals using the predict function so we calculate these by hand below.
```{r}
#| eval: false
...
linkinv <- family(coastal_ecosystem_model)$linkinv
coastal_model_se <- pred_grid |> 
  mutate(
    # get logit(mu)
    logit_mu = coastal_model_se$fit,
    # 95% in link space
    logit_mu_se = coastal_model_se$se.fit,
    logit_mu_lwr = qnorm(0.025, mean = logit_mu, sd = logit_mu_se),
    logit_mu_upr = qnorm(0.975, mean = logit_mu, sd = logit_mu_se),
    # undo the link function using the linkinv
    mu = linkinv(logit_mu),
    mu_lwr = linkinv(logit_mu_lwr),
    mu_upr = linkinv(logit_mu_upr)
  )
```
...
ggplot(coastal_model_se, aes(x = pc_gdp_2016, y = mu))+
...
```{r}
# low_boot_ci <- round(quantile(boot$pc_gdp_2016, probs = 0.025, na.rm = TRUE), 2)
# high_boot_ci <- round(quantile(boot$pc_gdp_2016, probs =  0.975, na.rm = TRUE), 2)
```
