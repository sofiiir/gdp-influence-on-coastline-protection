...
ggplot(coastal_model_pred, aes(x = pc_gdp_2016)) +
  geom_ribbon(data = boot_ci, aes(ymin = adj_prop_protected_lwr, ymax = adj_prop_protected_upr, fill = ecosystem)) +
  geom_line(aes(color = ecosystem, y = predict)) 
```
CI on the prediction is showing the confidence intervals when all of the variables influence are accounted for as opposed to the CI of the individual predictors showing the CI only based on that one variable. 
```{r}
