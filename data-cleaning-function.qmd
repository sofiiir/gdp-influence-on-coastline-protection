...
dfs <- c(coastline, coldwater_coral, mangroves, saltmarshes, seagrasses, warm_coral)
colors_list <- c("#773344", "#D77A61", "forestgreen", "#273E47", "#9BC1BC", "#ED6A5A")

clean_data <- function(df, column_name, gdp){

  # join df and gdp data
  df_gdp <- full_join(df, gdp, by = 'name')
...
```{r}
clean_data(df = coastline, column_name = total_coastal_length, gdp =  gdp)
```
```{r}
clean_data(df = mangroves, column_name = mangroves$total_area_2016, gdp = gdp)
```
